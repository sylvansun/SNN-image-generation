...
from models.snn_conv_gan import Generator, Discriminator
...
def main(args):
    ...
    for epoch in range(n_epochs):
        ...
        for i, (imgs, _) in enumerate(train_bar):
            ...
            _, gen_imgs = generator(z) # num_steps * BS * 28^2
            g_loss = 0
            for step in range(start_grad_step, num_steps):
                _, pred = discriminator(gen_imgs[step])
            ...
            d_loss = 0
            for step in range(start_grad_step, num_steps):
                _, real_pred = discriminator(real_imgs.reshape(batch_size, -1))
                _, fake_pred = discriminator(gen_imgs[step])
